...
T = {
    ...
    'close_folder_msg': "Zamknięto projekt." if is_pl else "Project closed."
}
...
<head>
    ...
    <style>
        ...
        .sidebar-title { padding: 10px 15px; font-size: 11px; text-transform: uppercase; color: #888; border-bottom: 1px solid #222; letter-spacing: 1px; flex-shrink: 0; user-select: none; display: flex; justify-content: space-between; align-items: center; }
        .close-folder-btn { cursor: pointer; font-size: 15px; font-weight: bold; padding: 0 6px; border-radius: 4px; transition: 0.2s; line-height: 1.2; }
        .close-folder-btn:hover { background: #333; color: #f44336; }
...
<body>
    ...
    <div id="main-body">
        <div id="sidebar">
            <div class="sidebar-title">
                <span id="txt-sidebar-title">Eksplorator</span>
                <span class="close-folder-btn" id="btn-close-folder" onclick="closeFolderAction()">×</span>
            </div>
    ...
    <script>
        ...
        const UI = {
            ...
            closeFolderAct: isEN ? "Close Folder" : "Zamknij folder",
        ...
        if (!isMac) {
            ...
            <div class="menu-item">${UI.mFile}
                <div class="dropdown">
                    ...
                    <div class="drop-item" onclick="closeFolderAction()">${UI.closeFolderAct}</div>
        ...
        document.getElementById('btn-close-folder').title = UI.closeFolderAct;
        ...
        function closeProjectUI() {
            document.getElementById('sidebar').style.display = 'none';
            document.getElementById('file-tree-content').innerHTML = '';
            if(editor) { setTimeout(() => editor.layout(), 50); }
        }

        function closeFolderAction() {
            if (window.pywebview && window.pywebview.api) {
                pywebview.api.close_folder();
            }
        }
...
class BackendApi:
    ...
    def close_folder(self):
        ...
        if APP_WINDOW:
            try:
                APP_WINDOW.evaluate_js("closeProjectUI()")
                self.print_terminal(f"\n[EditCode] {T['close_folder_msg']}\n")
            except: 
                pass
    ...
    if platform.system() == 'Darwin':
        ...
        menu_items = [
            Menu('Plik' if is_pl else 'File', [
                ...
                MenuAction("Zamknij folder", lambda: APP_WINDOW.evaluate_js('setTimeout(closeFolderAction, 10)')),
